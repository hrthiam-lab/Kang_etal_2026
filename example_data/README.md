...
Try NucMetrics using the example images provided in the [`example_data/`](../example_data/) folder.

### Test 1: Single nucleus — Mode 1 (Current Selection)
...
### Test 2: Single nucleus — Mode 3 (Binary Mask)

1. Open `example_data/compact.tif` in Fiji
2. Also open `example_data/compact_mask.tif`
3. **Click on the `compact.tif` window to make it active**
4. Click the NucMetrics icon
5. Select **Binary Mask** mode → select `compact_mask.tif` from the dropdown → click OK
6. Check the Results table for CV, 1-Gini, and DSI values

You can repeat with `decompact.tif` and `decompact_mask.tif` to compare metric values between compact and decompact chromatin states.

### Test 3: Multiple nuclei — Mode 4 (Auto-Generate Binary Mask)
...
> **⚠️ Make sure the original DNA-stained image window is active** (click on it) before running NucMetrics. If another window such as a mask is active, NucMetrics will treat it as the input image and the mask will not appear in the dropdown.
